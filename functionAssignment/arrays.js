...
const utme2024Students = [
  ...
  const californiaStudents = utme2024Students.filter((each, index)=>{
  ...
  const femaleStudents = utme2024Students.filter((each, index)=>{
    return each.gender === 'Female';
  })
  console.log(femaleStudents)

    
  // 3 = return the firstName and score
  const firstNameAndScore = utme2024Students.map((each)=>{
  ...
  let average = utme2024Students.reduce((currentScore, studentsScore)=>{
    return currentScore + studentsScore.score / utme2024Students.length
